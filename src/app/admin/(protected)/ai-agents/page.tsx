...
export default function AdminAiAgentsPage() {
  ...
  const aiCatId = catsData?.data?.find((c) => c.slug === AI_AGENT_SLUG)?.id

  const { data, isLoading, mutate } = useSWR(
    aiCatId ? ['admin-ai-agents', search, statusFilter, aiCatId] : null,
    () =>
      adminGetPosts({
        page: 1,
        limit: LIMIT,
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminPageHeader
        title="Sản phẩm AI Agent"
        description={`${products.length} sản phẩm hiển thị tại /xaykenh. Đổi "Thứ tự" để sắp xếp lại trên trang public. Có Badge mới được lên trang chủ.`}
        showBack={false}
      >
        <Link
      ...
      <div className="admin-card overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            title="Chưa có sản phẩm AI Agent nào"
            description='Bấm "Tạo mới", chọn danh mục "Phần mềm AI Agent" và điền nội dung.'
          />
        ) : (
              ...
              {products.map((post) => {
                ...
                return (
                  <tr key={post.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/ai-agents/${post.id}/edit`} className="text-slate-900 font-semibold hover:text-indigo-600">
                        {post.title}
                      </Link>
